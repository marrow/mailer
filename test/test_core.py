...
class TestMethods(TestCase):
    def test_startup(self):
        ...
        self.assertEqual(messages[0], "Mail delivery service starting.")
        self.assertEqual(messages[-1], "Mail delivery service started.")

        interface.start()

        self.assertEqual(len(messages), 6)
        self.assertEqual(messages[-1], "Attempt made to start an already running Mailer service.")
    ...
    def test_shutdown(self):
        ...
        self.assertEqual(len(messages), 5)
        self.assertEqual(messages[0], "Mail delivery service stopping.")
        self.assertEqual(messages[-1], "Mail delivery service stopped.")

        interface.stop()

        self.assertEqual(len(messages), 6)
        self.assertEqual(messages[-1], "Attempt made to stop an already stopped Mailer service.")
    ...
    def test_send(self):
        ...
        self.assertEqual(messages[0], "Attempting delivery of message foo.")
        self.assertEqual(messages[-1], "Message foo delivered.")

        message_fail = Bunch(id='bar', die=True)
        self.assertRaises(Exception, interface.send, message_fail)

        self.assertEqual(messages[-4], "Attempting delivery of message bar.")
        self.assertEqual(messages[-3], "Acquired existing transport instance.")
        self.assertEqual(messages[-2], "Shutting down transport due to unhandled exception.")
        self.assertEqual(messages[-1], "Delivery of message bar failed.")
